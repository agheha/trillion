			...
			<div class="buttons">
				<c:choose>
					<c:when test="${board.email == user.email}">
						<button type="button"
							onclick="location.href='<c:url value="/boardfree/freeupdateform.do?num=${board.num}"/>' ">수정</button>
						<button type="button"
							onclick="location.href='<c:url value="/boardfree/freedelete.do?num=${board.num}"/>' ">삭제</button>
					</c:when>
					<c:otherwise>
						<button type="button"
						...
						<form action='<c:url value="/admin/report.do"/>' method="post"
							...
							<div>
								...
								<input type="hidden" name="boardNum" value="${board.num}"
									...
									type="hidden" name="studyNum" value="0" class="studyNum" />
							</div>
							<div id="rpbtn">
